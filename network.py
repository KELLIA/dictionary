...
def network_main(form, con=None):
    ...
    with con:
        cur = con.cursor()
        cur.execute("select pos, word, phrase, freq, dialect from networks where pos=? and word=? order by freq DESC limit 20", (pos,word))
        rows = cur.fetchall()
        if len(rows) < 1:
            rows = []
            page = """<div class="content">No network data found for your query</div>"""
    ...
    output = []
    if any([row[-1] == "sahidic" for row in rows]):
        dialect = "sahidic"
    else:
        dialect = "bohairic"  # Bohairic only entry
    for row in rows:
        if row[-1] == dialect:
            output.append("||".join(list(row)[:-2] + [str(row[-2])]))

    tsv = "%%".join(output) + "\n"
    page = page.replace("**DATA**",tsv)
    page = page.replace("**WORD**",word)

    wrapped = wrap(page, caller="network")
    ...
    wrapped = wrapped.replace('<link rel="canonical" href="https://coptic-dictionary.org/" />','<link rel="canonical" href="https://coptic-dictionary.org/network.py" />')
    if tla != "--":
